...
class MainWindow: #class running the whole program 
    def __init__(self, master): #when the main class is initiated, the init function runs and creates the GUI for the main window
        ...
        ttk.Button(self.EntryFrame, text= "Search All", command= self.search_window).grid(row =4, column= 0, columnspan= 2, padx=5, pady = 5)
        ...
        ttk.Label(SearchFrame,text="Receipt:").pack(side=LEFT, padx= 5)
        ...
        DisplayFrame=ttk.LabelFrame(frame, text= "Receipt Display", style= "sub.TFrame")
        DisplayFrame.pack(**padding, expand=TRUE,fill = BOTH)
        
        
        
        self.receipt_image = ttk.Label(DisplayFrame, text="No receipts\n\n\n\n")
        self.receipt_image.pack( anchor= CENTER,expand= TRUE, pady= 5)
        ttk.Button(DisplayFrame, text= "Search All", command= self.search_window).pack( anchor= CENTER,expand= TRUE, ipadx= 10)
