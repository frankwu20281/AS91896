...
class MainWindow: 
    ...
    def search_window (self):   #function to initiate advanced search window 
        ...
        self.SW_style = ttk.Style(SearchWindow)
        ...
        ttk.Label(SearchWindow, text='Advanced Search').pack()
    ...
    def search_choice(self, event): #check what search option user wants
        ...
        if choice == "Name":
            self.search(self.adv_search_entry.get().strip(),  0, True )
        elif choice =="Receipt":
            #MainWindow.receipt_search(self, self.adv_search_entry.get(), TRUE) 
            self.search(self.adv_search_entry.get().strip(),  False, True )
        elif choice == "Item":
            MainWindow.search(self,self.adv_search_entry.get().strip(), 1, True)
        elif choice == "Search All":
            
            MainWindow.all_search(self, self.adv_search_entry.get())

    def all_search(self, typed):   #search all function for the advanced search window 
       
        print('allsearchstart')
        search_list =[]
         #see what user has typed
